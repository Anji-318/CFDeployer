    ...
    public class DeployService
    {
        ...
        public DeployService(string proxyUrl, string proxyKey)
        {
            ...
            if (!string.IsNullOrWhiteSpace(_proxyUrl) &&
                (!Uri.TryCreate(_proxyUrl, UriKind.Absolute, out var uri) ||
        ...
        {
            // Pages 部署走直连服务，无需代理
            if (job.DeployTarget == DeployTarget.Pages)
            {
                return await PagesDeployService.DeploySingleAsync(ConvertToPagesJob(job)).ConfigureAwait(false);
            }

            // Worker 部署需要代理
            if (string.IsNullOrWhiteSpace(_proxyUrl))
            {
                return new DeployResult { Success = false, Error = "代理URL未配置。Worker部署必须通过Cloudflare Worker代理进行。" };
            }
        ...
        {
            var results = new DeployResult[jobs.Count];

            // 如果包含 Pages 部署，限制并发不超过 2，避免触发 Cloudflare 限流
            bool hasPages = jobs.Any(j => j.DeployTarget == DeployTarget.Pages);
            int actualConcurrency = hasPages ? Math.Min(concurrency, 2) : concurrency;
            ...
            for (int i = 0; i < jobs.Count; i++)
            {
                ...
                var task = Task.Run(async () =>
                {
                    ...
                    try
                    {
                        ...
                        if (job.DeployTarget == DeployTarget.Pages)
                        {
                            result = await PagesDeployService.DeploySingleAsync(ConvertToPagesJob(job)).ConfigureAwait(false);
                        }
                        else
                        {
                            result = await DeploySingleAsync(job).ConfigureAwait(false);
                        }
        ...
        private string? ValidateJob(DeployJob job)
        {
            ...
            if (job.DeployTarget == DeployTarget.Pages)
            {
                if (string.IsNullOrWhiteSpace(job.PagesProjectName)) return "Pages项目名不能为空";
                return null;
            }
        ...
        private static PagesDeployJob ConvertToPagesJob(DeployJob job)
        {
            return new PagesDeployJob
            {
                AccountId = job.AccountId.Trim(),
                ApiToken = job.ApiToken.Trim(),
                ProjectName = job.PagesProjectName.Trim(),
                Branch = string.IsNullOrWhiteSpace(job.PagesBranch) ? "main" : job.PagesBranch.Trim(),
                StaticDir = job.PagesStaticDir,
                Script = job.Script,
                DeployType = job.PagesDeployType,
                EnvironmentVariables = job.EnvironmentVariables ?? new Dictionary<string, string>()
            };
        }
    }
